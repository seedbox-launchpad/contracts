...
contract TiersV2 is IERC721Receiver, Ownable, ReentrancyGuard {
    ...
    constructor(address _pkf, address _sPkf, address _uniLp, address _penaltyWallet) {
        ...
        addExternalToken(_sPkf, 2, 102, false, true);
        ...
        returns (uint8 res)
    {
        ...
        for (uint8 i = 0; i < tokenAddresses.length; i++) {
          ...
          ExternalToken storage token = externalToken[_token];
          totalStaked = totalStaked.add((userInfo[_userAddress][_token].staked).mul(token.rate).div(10**token.decimals));
        }
