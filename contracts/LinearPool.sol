...
contract LinearPool is Initializable, ILinearPool, OwnableUpgradeable {
    ...
    // The accepted token
    IERC20Upgradeable public linearAcceptedToken;
    ...
    // The common amount of SBX in staking;
    uint256 public override commonAmount;
     ...
     */
    function __LinearPool_init(IERC20Upgradeable _acceptedToken) internal {
        __Ownable_init();
    ...
    /**
     * @notice Gets number of deposited tokens of user from all pools
     * @param _account address of a user
     * @return amount total token deposited in all pools by a user
     */
    function commonBalanceOf(address _account)
        external
        view
        override
        returns (uint256)
    {   
        ...
        for (uint i = 0; i < linearPoolInfo.length; i++) {
            amount += linearBalanceOf(i, _account);
        }
        return amount;
    }
